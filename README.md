...
- **🪟 Windows 桌面端** (`src/`)：
  ...
  - 右键菜单支持一键开启/关闭开机自启。
- **🐧 Linux 桌面端** (`linux/`)：
  - **顶部菜单栏常驻** (`linux/codex-monitor-tray`)：遵从 Linux 状态栏规范，在顶部菜单栏常驻显示 **`⚡ 68% (5d)`**，点击弹出暗黑下拉详情菜单。
  - **终端独立程序源码** (`linux/main.go`)：纯 Go 静态编译，专为无桌面 Docker 与 SSH 服务器打造。
...
- **Linux Native Top Bar & CLI** (`linux/`):
